...
install.packages("zoo") ; install.packages("dplyr") ; install.packages("lubridate") ; install.packages('ggforce') ; install.packages('tidyverse') ; install.packages('ggplot2')
library(zoo) ; library(dplyr) ; library(lubridate) ; library(ggforce) ; library(tidyverse) ; library(ggplot2)
...
# IMPORTANT: Before running this script, ensure you've unzipped the repository and set the working directory to the folder where the data files are stored.
# Replace the path below with the location on your computer where you have unzipped the repository files.
setwd("/path/to/your/unzipped/repository")  # <-- Change this to the path where you unzipped the data and scripts
...
L1.GPS <- read.delim("L1.GPS.csv") #Change directory path as appropriate
